...
pl=plot(ax,n3,squeeze(bif(:,:,2)),'-','LineWidth',2);
myTxtFmt(xlabel(ax,'Strength of conditioning (\eta_3)'),20,0);
myTxtFmt(ylabel(ax,'Bifurcation point (\epsilon_3)'),20,0);
%legend(pl,{'\omega_3=1.5','\omega_3=2.5','\omega_3=3'},'Location','northwest','FontSize',10,'AutoUpdate','off');
legend(pl,{'\omega_3=0.5','\omega_3=1.0','\omega_3=2.5'},'Location','northwest','FontSize',15,'AutoUpdate','off');
set(ax,'ColorOrderIndex',1);
plot(ax,n3,squeeze(bif(:,:,1)),'--','LineWidth',2);
%plot(ax,n3,e2*ones(length(n3),1),'--k');
